...
DOCUMENTATION = r'''
---
name: nvidia.infra_controller.nico
short_description: Dynamic inventory from NVIDIA Infra Controller
description:
    - Queries the NVIDIA Infra Controller API for instances and builds
    ...
    - Reuses the same HTTP client and auth as the nvidia.infra_controller modules.
version_added: "1.0.0"
author: Fabien Dupont

options:
    plugin:
        description: Must be C(nvidia.infra_controller.nico).
        required: true
        choices: ['nvidia.infra_controller.nico']
    api_url:
        description: URL of the NVIDIA Infra Controller API.
        type: str
        required: true
        env:
...
# Also create groups from labels (e.g., nico_label_cluster_nvl72)
...
class _InventoryModule(object):
    """Thin adapter that looks like an AnsibleModule to NicoClient."""
...
class InventoryModule(BaseInventoryPlugin, Constructable):
    ...
    def verify_file(self, path):
        """Accept any YAML file that could be a NICo inventory source."""
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        if not api_url or not api_token or not org:
            raise AnsibleParserError(
                ...
                '(set in the inventory file or via NVIDIA_NICO_* env vars)'
            )
        ...
        for raw_instance in instances:
            ...
            self.inventory.set_variable(name, 'nico', instance)

            for field in ('id', 'site_id', 'vpc_id', 'instance_type_id',
                          'machine_id', 'operating_system_id', 'status'):
                val = instance.get(field)
                if val is not None:
                    self.inventory.set_variable(name, 'nico_%s' % field, val)

            labels = instance.get('labels') or {}
            self.inventory.set_variable(name, 'nico_labels', labels)
    ...
    def _populate_topology(self, client, group_prefix):
        ...
        if tenant:
            ...
            self.inventory.add_group(group_name)
            self.inventory.set_variable(group_name, 'nico_tenant_id', tenant_id)
            self.inventory.set_variable(group_name, 'nico_tenant_name', tenant_name)
        ...
        for raw_site in sites:
            ...
            self.inventory.add_group(group_name)
            self.inventory.set_variable(group_name, 'nico_site_id', site_id)
            self.inventory.set_variable(group_name, 'nico_site_name', site_name)
        ...
        for raw_vpc in vpcs:
            ...
            self.inventory.add_group(group_name)
            self.inventory.set_variable(group_name, 'nico_vpc_id', vpc_id)
            self.inventory.set_variable(group_name, 'nico_vpc_name', vpc_name)
        ...
        for raw_it in instance_types:
            ...
            self.inventory.add_group(group_name)
            self.inventory.set_variable(group_name, 'nico_instance_type_id', it_id)
            self.inventory.set_variable(group_name, 'nico_instance_type_name', it_name)
        ...
        for raw_alloc in allocations:
            ...
            self.inventory.add_group(group_name)
            self.inventory.set_variable(group_name, 'nico_allocation_id', alloc_id)
            self.inventory.set_variable(group_name, 'nico_allocation_name', alloc_name)
        ...
        for raw_ib in ib_partitions:
            ...
            self.inventory.add_group(group_name)
            self.inventory.set_variable(group_name, 'nico_ib_partition_id', ib_id)
            self.inventory.set_variable(group_name, 'nico_ib_partition_name', ib_name)
